...
public:
	...
	ReturnCode_t delete_contentfilteredtopic(ContentFilteredTopic*& pContentFilteredTopic);
...
public:
	...
	/* Nephalem 获取内置订阅者 */
	Publisher* get_builtin_publisher();

	ReturnCode_t register_type(const string & typeName);
	/* Nephalem 注册数据类型 */
	ReturnCode_t register_type(const string & typeName,list<string>& strList);
