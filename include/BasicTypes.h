...
typedef  unsigned int SampleStateMask;
...
typedef  unsigned int ViewStateMask;
...
typedef  unsigned int InstanceStateMask;
...
struct Time_t 
{
	...
	//Time_t(int lTempSecond, unsigned int ulTempNanosec)
	//{
	//	this->sec = lTempSecond;
	//	this->nanosec = ulTempNanosec;
	//}

	//Time_t(const Time_t & right)
	//{
	//	sec = right.sec;
	//	nanosec = right.nanosec;
	//}
    ...
    bool operator <= (const Time_t &Right) const 
    {
        ...
        else if (this->sec < Right.sec) 
        {
            return true;
        }
        else if (this->nanosec > Right.nanosec) 
        {
            return false;
        }
        else 
    ...
    //{
    //    Time_t Temp = *this;
    //    if (Right.nanosec + Temp.nanosec >= (unsigned long)-1) 
    //    {
    //        ++ Temp.sec;
    //    }
    //    Temp.nanosec += Right.nanosec;
    //    Temp.sec += Right.sec;
    ...
    //{
    //    Time_t Temp = *this;
    //    if (Temp.nanosec - Right.nanosec < 0) 
    //    {
    //        -- Temp.sec;
    //    }
    //    Temp.nanosec -= Right.nanosec;
    ...
    {
        Time_t Temp = *this;
        if (Right.nanosec + Temp.nanosec >= 1000000000) 
        {
            ++ Temp.sec;
			Temp.nanosec = Temp.nanosec+Right.nanosec-1000000000;
        }
		else
			Temp.nanosec = Temp.nanosec+Right.nanosec;
    ...
    Time_t operator - (const Time_t &Right) const 
    {
        Time_t Temp = *this;
        //if (Temp.nanosec - Right.nanosec < 0) 
		if (Temp.nanosec < Right.nanosec) 
        {
            -- Temp.sec;
			Temp.nanosec = Temp.nanosec+1000000000-Right.nanosec;
        }
		else
			Temp.nanosec -= Right.nanosec;
	...
	Time_t operator / (const unsigned long &Right) const 
    {
        Time_t Temp = *this;
		Time_t Result;
		//double dTempb = Temp.sec + Temp.nanosec / 1000000000.0;
        //double dTempa = dTempb/Right;
        //Temp.sec = floor(dTempa);
		//Temp.nanosec = (dTempa - floor(dTempa))*1000000000;
		Result.sec = (Temp.sec*1000000000+Temp.nanosec)/(Right*1000000000);
		Result.nanosec = ((Temp.sec*1000000000+Temp.nanosec)-((Temp.sec*1000000000+Temp.nanosec)/(Right*1000000000))*(Right*1000000000))/Right;
        return Result;
    }
};
...
inline static bool GUID_PREFIX_EQUAL(const guidPrefix_t GuidPrefixA, const guidPrefix_t GuidPrefixB)
{
	for (int i = 0; i < 12; i++)
	{
		if (GuidPrefixA[i] != GuidPrefixB[i])
			return false;
	}
	return true;
}
...
#define PID_STATIC_MATCH_PARTICIPANT_MD5               0x8101
#define PID_STATIC_MATCH_PARTICIPANT_NAME              0x8102
...
struct GUID_t
{
	...
	{
		memcpy(this->guidPrefix, Right, sizeof(guidPrefix_t));
		return *this;
	}
...
struct _CACHE_CHANGE
{
	...
	_CACHE_CHANGE() :uiLength(0), pDatavalue(NULL), uiFragNum(0),uiFragTotal(0), valid_data(true), LifeSpan(TIME_INFINITE)
	{
	}

	~_CACHE_CHANGE()
	{
		lossFragNumber.clear();
	}
};
...
struct ChangeFromWriter
{
	...
	ChangeFromWriter() :status(UNKNOWN), is_relevant(true), pRelatedCacheChange(NULL) {}
...
struct ChangeForReader
{
	...
	ChangeForReader() :status(UNSENT), is_relevant(true)/*, pRelatedCacheChange(NULL) */{}
};
...
typedef list <ContentFilteredTopicImpl*> _CONTENT_FILTERED_TOPIC_IMPL_LIST_T;
