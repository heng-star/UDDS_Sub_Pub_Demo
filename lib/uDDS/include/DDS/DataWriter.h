...
public:
	...
	ReturnCode_t write_w_timestamp(const char* pData, unsigned int length, const InstanceHandle_t& handle, const Time_t& timestamp);
...
public:
	...
	/* Nephalem 扩展功能 */
	/* Nephalem 定向写入,发送数据给指定reader */
	ReturnCode_t directed_write(const char* pData, unsigned int length, const InstanceHandle_t & handle,const GUID_t& stReaderGuid);

	ReturnCode_t directed_write(const char* pData, unsigned int length, const InstanceHandle_t & handle, const GUID_t& stReaderGuid, _PARAMETER_LIST& InlineQos);

	ReturnCode_t write(const char* pData, unsigned int length, const InstanceHandle_t& handle, _PARAMETER_LIST& InlineQos);
